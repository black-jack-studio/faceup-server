...
// (client/src/pages/auth/welcome.tsx) — reused as-is rather than exporting a second bezel,
// just clipped down to its bottom slice below instead of shown whole.
...
const FULL_HEIGHT = FRAME.h * MOCK_SCALE;
// How much of the phone's bottom stays visible — the rest is clipped off the top so this reads
// as a sliced-open device rather than a shrunk whole one. Tune once real screenshots land.
const VISIBLE_FRACTION = 0.46;
const VISIBLE_HEIGHT = FULL_HEIGHT * VISIBLE_FRACTION;
const HIDDEN_TOP = FULL_HEIGHT - VISIBLE_HEIGHT;
...
export default function PhoneMockupFrame({ image, alt }: PhoneMockupFrameProps) {
  return (
    <div className="relative mx-auto overflow-hidden" style={{ width: MOCK_WIDTH, height: VISIBLE_HEIGHT }}>
      <div className="absolute left-0" style={{ top: -HIDDEN_TOP, width: MOCK_WIDTH, height: FULL_HEIGHT }}>
        <div
        ...
        <img
          ...
          style={{ width: MOCK_WIDTH, height: FULL_HEIGHT }}
        />
      </div>
    </div>
  );
