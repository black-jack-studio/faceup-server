...
// Play with Friends reuses the classic engine and economy under the hood — only the table
// visuals differ (two seats beside the player). Real multiplayer (inviting/joining friends)
// isn't built yet; the two seats are placeholders until then.
export default function PlayWithFriends() {
  ...
  const { placeBet, navigateToGame, isLoading } = useBetting({
    mode: "classic",
    onSuccess: (result) => {
      navigateToGame(result.gameId, result.betAmount, { mode: "classic", layout: "friends" });
    },
  });
    ...
    >
      <div className="max-w-md mx-auto relative h-full">
        ...
        <div className="h-full flex flex-col pb-6 gap-8">
          ...
          >
            ...
            >
              ...
              >
                ...
                <h1 className="text-lg font-medium text-white">Play with Friends</h1>
              </motion.div>
          ...
          {/* Friends note — real invites/matchmaking aren't built yet */}
          <motion.div
            className="flex-shrink-0 px-6 text-center"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.4, delay: 0.25 }}
          >
            <p className="text-white/40 text-xs">
              Inviting friends is coming soon — for now the two extra seats at the table are just for show.
            </p>
          </motion.div>
