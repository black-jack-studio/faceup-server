...
export const AVATAR_CATALOG: AvatarEntry[] = [
  ...
  // (Curly Hair), previously the very last entry (bottom-left, since an odd-length list's last
  // row only has a left column), moved up to take its place instead, so the row-4 slot isn't
  // just left empty. The remaining left-over "person" heads (no direct male/female match:
  // bald, beard, turban) trail at the end.
